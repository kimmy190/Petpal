...
const PetCard = ({ id, imageUrl, petName, availability, age, gender, breed, adoptionCenter, adoptionDate, editable=false, onEdit }) => {
    const options = {
        year: 'numeric',
        month: 'long',
        day: 'numeric'
    ...
    formattedDate = new Intl.DateTimeFormat('en-US', options).format(formattedDate);
    return (
        <div className="relative max-h-max max-w-xs md:max-w-sm bg-white shadow rounded-lg mb-2 transition duration-300 ease-in-out hover:shadow-lg hover:opacity-80">
          ...
          <a href="#">
            <h5 className="mb-2 text-xl font-semibold tracking-tight text-gray-900">{petName}</h5>
          </a>
              <div>
            ...
            <p className="text-medium text-gray-700">{gender} {'\u2022'} {age}</p>
            <p className="text-medium text-gray-700">{breed}</p>
            <p className="text-medium text-gray-700">{adoptionCenter}</p>
            <p className="text-xs mt-2  text-gray-700">{formattedDate.toString()}</p>
          </div>
        </div>
    );
};
