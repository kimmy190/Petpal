...
const breeds = ["Golden Retreiver", "German Shepherd", "Boston Terrier", "Labrador"]

function SearchGrid({fetchOnLoad, location, shelter_id, editable=false, onEdit}) {
...
// this is dumb w.e
    ...
    return (
        <div className="flex flex-col md:flex-row m-4 container md:max-w-full  bg-white rounded-lg shadow-md gap-4">
          <div id="drawer-disabled-backdrop" className={`${hideFilter} flex z-40 p-4 w-full md:w-[300px] flex-none bg-white`} tabIndex="-1" aria-labelledby="drawer-disabled-backdrop-label">

            <div className="flex flex-col w-full">
              <label htmlFor="species" className="mb-4 md:mb-2 mr-4 font-bold">Species</label>
              <Dropdown id="species" label={species === "" ? "Any species": species} color="dark" className="flex-shrink-0 z-10 inline-flex items-center py-3.5 ">
                {dropdownItems.map((item, index) => (
                    <Dropdown.Item key={index} onClick={() => handleSpeciesClick(item)}>
                      {item}
                    </Dropdown.Item>
                ))}
              </Dropdown>
              <hr className="mt-4 mb-2"/>
          <label htmlFor="breed" className="mb-4 md:mb-2 mr-4 font-bold">Species</label>
              <Dropdown label={breed === "" ? "Any breed": breed} color="dark" className="flex-shrink-0 z-10 inline-flex items-center py-3.5 ">
              ...
              <hr className="mt-4 mb-2"/>
              <label htmlFor="size" className="mb-4 md:mb-2 mr-4 font-bold">Size</label>
            ...
            <div className="md:grow px-8 pt-4  bg-white">
                <div className="flex flex-row flex-wrap justify-between items-center md:items-end">
                    <div className="flex flex-row items-center gap-x-4">
                      <Button onClick={toggleFilter} id="showDrawer" className="gray-800" color="dark" type="button" aria-controls="drawer-disabled-backdrop">
                  ...
                  <div className="gap-4 flex flex-row flex-wrap space-between md:items-end">
                    <div>
                      <label htmlFor="sort" className="mb-2 text-sm font-medium text-gray-900 dark:text-white">
                        Sort
                      </label>
                      <Select
                        id="sort"
                  ...
                  {editable && (
                        ...
                        <a href="/pet_creation"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5"
                                stroke="currentColor" className="block text-gray-700 w-full h-56">
