...
class SeekerSerializer(ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "profile_image",
            "shelter",
            "create_petlisting_notification",
        ]
...
class ShelterSerializer(WritableNestedModelSerializer):
    ...
    def update(self, instance, validated_data):
        with transaction.atomic():
            ...
            instance.shelter.organization_name = shelter_data.get(
                "organization_name", instance.shelter.organization_name
            )
            instance.shelter.logo_image = shelter_data.get(
                "logo_image", instance.shelter.logo_image
            )
            instance.shelter.phone_number = shelter_data.get(
                "phone_number", instance.shelter.phone_number
            )
            instance.shelter.mission_statement = shelter_data.get(
                "mission_statement", instance.shelter.mission_statement
            )
            instance.shelter.country = shelter_data.get(
                "country", instance.shelter.country
            )
            instance.shelter.address1 = shelter_data.get(
                "address1", instance.shelter.address1
            )
            instance.shelter.address2 = shelter_data.get(
                "address2", instance.shelter.address2
            )
            ...
            shelter_instance, _ = PetShelter.objects.update_or_create(
                user=instance, defaults=shelter_data
            )
            ...
            for image_data in images_data:
                ShelterImage.objects.update_or_create(
                    shelter=shelter_instance, defaults={"image": image_data}
                )
